...
# Date: 07.01.2021
# Description:
# This code verifies that ((0,1,0),(0,0,1)) is the unique Nash equilibrium of the HPG with the parameters selected in
# Section VI-A of the paper. In order to compile, the Z3 solver and the Z3Py (the Z3 API in Python) are required to be
# installed and properly set up. For installation and set up information on Z3 and its bindings, see
...
# Then, we enter the payoff to each population and strategy as determined by the HPG with the parameters selected in
# Section VI-A of the paper. The 'Fri' below represents the component of the payoff corresponding to population
...
# variables x11,...,x23 that belong to the simplex and satisfy the below conditions are Nash equilibria of the HPG with
# the parameters selected in Section VI-A of the paper.
...
# ((0,1,0),(0,0,1)) is a Nash equilibrium of the HPG with the parameters selected in Section VI-A of the paper. In order
# to see that this the unique Nash equilibrium, we enforce the condition to the solver that the social state is not
# equal to ((0,1,0),(0,0,1)), and run the resulting code. The condition that the social is not equal to
